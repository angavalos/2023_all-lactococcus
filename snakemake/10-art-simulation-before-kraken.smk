...
inpath = "/group/weimergrp2/genomes/lactococcus/genbank-sra-art-simulation" #where your genome sequences are
#trimpath = "/group/weimergrp2/genomes/lactococcus/analysis/sampleFiles" #where your successful trims are
outpath = "/group/weimergrp2/genomes/lactococcus/genbank-sra-art-simulation/analysis" #where you want output files
        ## edit number of threads you wish to give shovill (usually 4)

#fastq files common path and base name
sampleids=[]
for name in glob.glob(inpath+"/*.fna_1.fq"):
    ID = os.path.basename(name).split(".fna_1.fq")[0]
    if ID not in sampleids and os.path.isfile(inpath+"/"+ID+".fna_1.fq") == True and os.path.isfile(inpath+"/"+ID+".fna_2.fq") == True:
        sampleids.append(ID)
