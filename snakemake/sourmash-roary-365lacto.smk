...
filena = "/group/weimergrp/aavalos7/projects/all-lactococcus/final-lacto-gt90completion-lt5contamination-gt50percentlacto.txt"
...
sampleids = [i for i in lines if i not in lines_out]
#sampleids=["BCW-000213"]
#sampleids=[]
#totalids=[]
#for name in os.listdir(trimpath):
#    totalids.append(name)
#    if (name not in sampleids) and (os.path.isfile(trimpath+"/"+name+"/checkm/storage/bin_stats_ext.tsv")==True):
#        sampleids.append(name)
#print("Total: ",len(totalids))
#print("Have CheckM output: ",len(sampleids))
#print(sampleids)
...
rule all:
    input:
        outpath+"/population/sourmash-365lacto/population_sig_compare_31-100k-365lacto.matrix",
        outpath+"/population/sourmash-365lacto/population_sig_compare_31-100k-365lacto.matrix.matrix.png",
        outpath+"/population/roary-365lacto/gene_presence_absence-365lacto.csv"
...
rule sour_cmp_31:
    input:
        expand(outpath+"/sampleFiles/{smpl}/sourmash/{smpl}.sig", smpl=sampleids),
    output:
        o1 = outpath+"/population/sourmash-365lacto/population_sig_compare_31-100k-365lacto.csv",
        o2 = outpath+"/population/sourmash-365lacto/population_sig_compare_31-100k-365lacto.matrix"
    conda:
        "snakeprograms/ymlfiles/sourmash.yml"
    shell:
        '''
...
rule sour_plt:
    ...
    output:
        outpath+"/population/sourmash-365lacto/population_sig_compare_31-100k-365lacto.matrix.matrix.png"
    params:
        outpath+"/population/sourmash-365lacto/"
    conda:
        "snakeprograms/ymlfiles/sourmash.yml"
    shell:
        '''
...
rule roary:
    input:
        expand(outpath+"/sampleFiles/{smpl}/prokka/{smpl}.gff", smpl=sampleids),
    output:
        outpath+"/population/roary-365lacto/core_gene_alignment-365lacto.aln",
        outpath+"/population/roary-365lacto/gene_presence_absence-365lacto.csv"
    params:
        pt = outpath+"/population/roary-365lacto"
    conda:
