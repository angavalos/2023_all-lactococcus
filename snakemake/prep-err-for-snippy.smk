...
filename = "/group/weimergrp/aavalos7/projects/all-lactococcus/20231001_final-err-list.txt"
...
print("ERR SRA Samples: ",len(sampleids))
print(sampleids)
...
rule fixed:
    ...
    shell:
        r'''
        sed -E "s/^((@|\+)ERR[^.]+\.[^.]+)\.(1|2)/\1/" {input.i1} > {output.o1}
        sed -E "s/^((@|\+)ERR[^.]+\.[^.]+)\.(1|2)/\1/" {input.i2} > {output.o2}
        '''
